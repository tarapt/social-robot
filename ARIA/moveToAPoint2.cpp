...
void moveTo(ArRobot& robot, ArPose target, int speed) {
  ...
  ArLog::log(ArLog::Normal, "moveToAPoint: Driving towards the target at %d mm/s...", speed);
  robot.lock();
  int distance = robot.findDistanceTo(target) * 1000;
  ArLog::log(ArLog::Normal, "moveToAPoint: distance = %d, time = %d milliseconds", distance, distance/speed);
  robot.setRotVel(0);
  robot.setVel(speed);
  ArUtil::sleep(distance/speed);
  printStatus(robot);
