...
def quantize_and_convert(euler_angle):
    for i in range(3):
        euler_angle[i] = (
            (euler_angle[i] // quantization_step) + 0.5) * quantization_step
    return euler_angle
...
class HeadPoseEstimator:
    ...
    def get_head_pose(self, shape):
        ...
        return cube_image_coords, quantize_and_convert(rotationMatrixToEulerAngles(rotation_mat))

    def draw_head_pose(self, cube_image_coords, euler_angle, frame):
        for start, end in self.line_pairs:
            cv2.line(frame, cube_image_coords[start],
                     cube_image_coords[end], (0, 0, 255))
...
def main():
    ...
    while True:
        ...
        if len(face_rects) > 0:
            ...
            cube_image_coords, euler_angle = pose_estimator.get_head_pose(
                shape)
            frame = pose_estimator.draw_head_pose(
                cube_image_coords, euler_angle, frame)
